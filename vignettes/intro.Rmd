...
author: "Davide Risso"
date: "Last modified: February 15, 2018; Compiled: `r format(Sys.time(), '%B %d, %Y')`"
...
The factors inferred in the `zinbwave` model can be used to find subpopulations using Seurat's cluster analysis method.

Note that the following workflow has only been tested with Seurat's version 2.3.4.

Here we create a Seurat object starting from the `SingleCellExperiment` object. 
Note that the coercion function requires that a slot named `logcounts` is present. This slot should contain the normalized data, hence we rename our `normalizedValues` slot to `logcounts`.

Please, refer to the Seurat's vignettes for a typical analysis, which includes filtering, normalization, etc.

```{r seurat}
library(Seurat)
assayNames(fluidigm_norm)[5] <- "logcounts"
seu <- as.seurat(fluidigm_norm)
```

The object will include the `zinbwave` factors matrix as one of the dimensionality reduction slots, which we can use for cluster analysis.
